from kfp import dsl
from kfp.dsl import component, Input, Dataset
from _kubeflow.config import BASE_IMAGE

@component(
    base_image=BASE_IMAGE
)
def feast_sync_component(
    feast_data: Input[Dataset],
    feast_repo_path: str,
    minio_endpoint: str,
    minio_access_key: str,
    minio_secret_key: str,
):
    import os

    # ✅ boto3 / pyarrow env vars
    os.environ["AWS_ACCESS_KEY_ID"]       = minio_access_key
    os.environ["AWS_SECRET_ACCESS_KEY"]   = minio_secret_key
    os.environ["AWS_DEFAULT_REGION"]      = "us-east-1"
    os.environ["AWS_ENDPOINT_URL"]        = minio_endpoint
    os.environ["AWS_S3_ENDPOINT_URL"]     = minio_endpoint
    os.environ["AWS_S3_ENDPOINT"]         = minio_endpoint
    os.environ["FEAST_S3_ENDPOINT_URL"]   = minio_endpoint
    os.environ["AWS_S3_FORCE_PATH_STYLE"] = "true"
    os.environ["AWS_VIRTUAL_HOSTING"]     = "false"

    # ✅ s3fs / aiobotocore env vars (used by materialize_incremental)
    os.environ["FSSPEC_S3_ENDPOINT_URL"]  = minio_endpoint
    os.environ["FSSPEC_S3_KEY"]           = minio_access_key
    os.environ["FSSPEC_S3_SECRET"]        = minio_secret_key
    os.environ["FSSPEC_S3_CLIENT_KWARGS"] = f'{{"endpoint_url": "{minio_endpoint}"}}'

    # ✅ Import AFTER all env vars are set
    from src.model_development.feast_sync import sync_to_feast

    print(f"MinIO Configuration:")
    print(f"  Endpoint: {minio_endpoint}")
    print(f"  Access Key: {minio_access_key[:4]}...")
    print(f"  Region: us-east-1")

    uri = feast_data.uri.rstrip('/')

    if uri.startswith("minio://"):
        parquet_uri = f"s3://{uri[len('minio://'):]}/preprocessed_data.parquet"
    elif uri.startswith("s3://"):
        parquet_uri = f"{uri}/preprocessed_data.parquet"
    else:
        parquet_uri = os.path.join(feast_data.path, "preprocessed_data.parquet")

    print(f"Feast offline path (MinIO): {parquet_uri}")

    # ✅ Pass credentials directly into sync_to_feast
    sync_to_feast(
        parquet_path=parquet_uri,
        feast_repo_path=feast_repo_path,
        minio_endpoint=minio_endpoint,
        minio_access_key=minio_access_key,
        minio_secret_key=minio_secret_key,
    )